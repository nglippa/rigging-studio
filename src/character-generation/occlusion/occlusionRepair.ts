...
export const detectOcclusionReviews = (parts: readonly ProposedCharacterPart[]): readonly OcclusionReview[] => parts
  ...
  .map((part) => ({ partId: part.id, likelyOccluded: true, confidence: Math.max(.55, 1 - part.confidence), reason: part.warnings.join(" "), decision: "unreviewed", reconstructionAccepted: false }));
